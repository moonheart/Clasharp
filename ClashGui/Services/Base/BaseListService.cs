...
public abstract class BaseListService<T, TKey> : BaseService<List<T>>, IAutoFreshable, IObservableListService<T, TKey>
    where TKey : notnull
{
    ...
    protected BaseListService(IClashCli clashCli, IClashApiFactory clashApiFactory) : base(clashApiFactory, clashCli)
    {
        ...
        GetObservable().Subscribe(d => { _items.AddOrUpdate(d); });
    }
